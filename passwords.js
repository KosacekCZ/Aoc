let input = "6-10 s: snkscgszxsssscss\n" +
    "6-7 b: bbbbbxkb\n" +
    "2-4 n: nnnjn\n" +
    "1-2 j: jjjj\n" +
    "5-9 z: jgzzzqhbj\n" +
...
for (let i = 0; i < inputSplit.length; i++) {
    let strSplit = inputSplit[i].split(" ");
    let countSplit = strSplit[0].split("-");
    let min = parseInt(countSplit[0]);
    let max = parseInt(countSplit[1]);
    let char = strSplit[1].slice(0, 1);
    let password = strSplit[2];
    console.log(password);
    console.log(min + " " + max + " " + char);
    if ((password.charAt(min-1) == char && password.charAt(max-1) != char) || (password.charAt(min-1) != char && password.charAt(max-1) == char)) {
        validPassCount++;
        console.log("found!");
    }
}
console.log("passwords: " + validPassCount);